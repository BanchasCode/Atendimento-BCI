...
class Textlocal
{
	...
	private function _sendRequest($command, $params = array())
	{
		...
		else throw new Exception('Manipulador de solicitação inválido.');
	...
	private function _sendRequestCurl($command, $params)
	{
		...
		if ($rawResponse === false) {
			throw new Exception('Falha ao conectar ao serviço Textlocal: ' . $error);
		} elseif ($httpCode != 200) {
			throw new Exception('Resposta ruim do serviço Textlocal: código HTTP ' . $httpCode);
		}
	...
	{
		throw new Exception('Método de transferência não suportado');
	}
	...
	public function sendSms($numbers, $message, $sender, $sched = null, $test = false, $receiptURL = null, $custom = null, $optouts = false, $simpleReplyService = false)
	{
		...
		if (!is_array($numbers))
			throw new Exception('Formato $numbers inválido. Deve ser uma matriz');
		if (empty($message))
			throw new Exception('messagem vazia');
		if (empty($sender))
			throw new Exception('Nome do remetente vazio');
		if (!is_null($sched) && !is_numeric($sched))
			throw new Exception('Formato de data inválido. Usar formato numérico de época.');
